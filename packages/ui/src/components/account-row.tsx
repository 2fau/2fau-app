...
export function AccountRow({ account, onEdit }: { account: Account; onEdit: () => void }) {
  ...
  const initial = (account.issuer || account.label || "?").charAt(0).toUpperCase();
  ...
  return (
    ...
    >
      <ItemMedia
        className="size-9 rounded-full border-transparent bg-muted text-[13px] font-semibold text-foreground"
        style={{ background: accountColorAccent(account.color), alignSelf: "center" }}
      >
        {initial}
      </ItemMedia>
